...
async function main() {
  console.log("🌱 Seeding Sage...")

  const ownerPassword = process.env.OWNER_PASSWORD || "memory2024"
  const passwordHash = await bcrypt.hash(ownerPassword, 10)

  const user = await prisma.user.upsert({
    where: { id: "owner" },
    update: { name: "admin", password: passwordHash },
    create: { id: "owner", name: "admin", password: passwordHash },
  })
  ...
  const tags = await Promise.all([
    prisma.tag.upsert({
      ...
      update: {},
      create: { userId: user.id, name: "工作", slug: "work", color: "#6366f1", sortOrder: 0 },
    }),
    prisma.tag.upsert({
      where: { userId_slug: { userId: user.id, slug: "personal" } },
      update: {},
      create: { userId: user.id, name: "个人", slug: "personal", color: "#22c55e", sortOrder: 1 },
    }),
    prisma.tag.upsert({
      where: { userId_slug: { userId: user.id, slug: "health" } },
      update: {},
      create: { userId: user.id, name: "健康", slug: "health", color: "#ef4444", sortOrder: 2 },
    }),
    prisma.tag.upsert({
      where: { userId_slug: { userId: user.id, slug: "learning" } },
      update: {},
      create: { userId: user.id, name: "学习", slug: "learning", color: "#f97316", sortOrder: 3 },
    }),
    prisma.tag.upsert({
      where: { userId_slug: { userId: user.id, slug: "finance" } },
      update: {},
      create: { userId: user.id, name: "财务", slug: "finance", color: "#eab308", sortOrder: 4 },
    }),
  ])
  ...
  for (const taskSeedItem of taskSeed) {
    const { tagIds, ...data } = taskSeedItem
    const task = await prisma.task.create({
      data: {
        userId: user.id,
        sortOrder: createdTasks.length,
        ...data,
      },
    })
  ...
  await prisma.subTask.createMany({
    data: [
      { taskId: createdTasks[0], title: "整理需求", sortOrder: 0 },
      { taskId: createdTasks[0], title: "写出一版提纲", sortOrder: 1 },
      { taskId: createdTasks[1], title: "确认收件人", sortOrder: 0, done: true },
    ],
  })
    ...
    data: [
      {
        ...
        endAt: addHours(addDays(new Date(), 1), 1),
      },
      {
        taskId: createdTasks[4],
        startAt: addDays(new Date(), 2),
        endAt: addHours(addDays(new Date(), 2), 2),
      },
    ],
  })
  ...
  for (const noteSeedItem of noteSeed) {
    const { tagIds, ...data } = noteSeedItem
  ...
  await prisma.noteTask.createMany({
    data: [
      { noteId: createdNotes[0], taskId: createdTasks[0] },
      { noteId: createdNotes[1], taskId: createdTasks[2] },
    ],
  })

  await prisma.inboxItem.createMany({
    data: [
      { userId: user.id, content: "和朋友讨论的那个副业想法：做一个 AI 写作助手工具", capturedAt: new Date() },
      { userId: user.id, content: "买一本《深度工作》，感觉最近很难专注", capturedAt: subDays(new Date(), 1) },
      { userId: user.id, content: "考虑学一门新语言，Rust 还是 Go？", capturedAt: subDays(new Date(), 2) },
    ],
  })

  await prisma.setting.upsert({
    where: { userId_key: { userId: user.id, key: "app_preferences" } },
    update: {
      value: {
        language: "zh-Hans",
        theme: "system",
  ...
  console.log(`   - 1 user (admin / ${ownerPassword})`)
  console.log(`   - ${tags.length} tags`)
  console.log(`   - ${taskSeed.length} tasks`)
  console.log(`   - ${noteSeed.length} notes`)
